...
from bot.keyboards import create_ad_callback, send_contact_keyboard, category_ad_keyboard, home_category_callback, \
    work_category_callback, goods_category_callback, \
    services_category_callback, validate_keyboard, start_keyboard, return_to_start_callback, return_to_start_keyboard, \
    start_keyboard_admin, conversation_skip_image_callback, conversation_skip_image_keyboard, publish_ad_keyboard
from database.models import CreateAdRequest
from database.services import check_user_phone_is_none, create_ad, fetch_category_by_alias, fetch_user_by_id, \
...
def conversation_handler():
    return ConversationHandler(
        ...
        states={
            TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, conversation_title)],
            ...
            CATEGORY: [
                CallbackQueryHandler(conversation_category, pattern=home_category_callback),
                CallbackQueryHandler(conversation_category, pattern=work_category_callback),
                CallbackQueryHandler(conversation_category, pattern=goods_category_callback),
                CallbackQueryHandler(conversation_category, pattern=services_category_callback)
            ],
            IMAGE: [
                MessageHandler(filters.PHOTO, conversation_image),
                CallbackQueryHandler(conversation_image, pattern=conversation_skip_image_callback)
            ],
            SEND: [
                MessageHandler(filters.PHOTO, conversation_publish),
            ]
        },
        fallbacks=[CallbackQueryHandler(conversation_cancel, pattern=return_to_start_callback)], allow_reentry=True
    )
    ...
    if user_id:
        ...
        if phone_is_none:
            await context.bot.send_message(
                ...
                text='Чтобы добавить объявление, нужно указать телефон. Пожалуйста, нажмите на кнопку.',
...
async def process_images_handler(context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(files) > 5:
        await context.bot.send_message(
            ...
            text='Максимальное количество фото - 5 штук'
        )
    else:
...
async def conversation_publish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        if file_id:
            ...
            await context.bot.send_message(
                chat_id='1472373299',
                text=f'Подтвердите или отклоните объявление',
                parse_mode=ParseMode.HTML,
                reply_markup=validate_keyboard(create_ad_response.ad_id)
            )
        else:
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text='Ваше объявление принято и после модерации будет опубликовано в канале.\n'
